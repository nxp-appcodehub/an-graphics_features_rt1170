...
#include "CoolingPXP.h"
#include "LightsPXP.h"
#include "LockPXP.h"
#include "SpeakerPXP.h"
...
AT_NONCACHEABLE_SECTION_ALIGN(static outputPixel_t s_BufferLcd[2][APP_IMG_HEIGHT][APP_IMG_WIDTH], FRAME_BUFFER_ALIGN);

//Buffers to use as PS:
AT_NONCACHEABLE_SECTION_ALIGN(static uint16_t pxpSpeaker[155][128], FRAME_BUFFER_ALIGN);
...
static void ConfigureOB(int width, int height, void * address)
{
  ...
  outputBufferConfig.pitchBytes     = APP_IMG_WIDTH * DISPLAY_BPP;
...
static void APP_InitLcdif(void)
{
  ...
  fbInfo.strideBytes = APP_IMG_WIDTH * DISPLAY_BPP;
...
static void APP_InitPxp(void)
{
  ...
  memset(s_BufferLcd, 0xFFFFFFFFU, sizeof(s_BufferLcd));
  memset(pxpSpeaker, 0, 155*128*2);
...
void DrawRadialBars(vg_lite_buffer_t * target)
{
...
static void redrawPXP()
{
  ...
  ConfigureOB(720,1280, (void *)s_BufferLcd[curLcdBufferIdx]);

  //Complete Background rendering
  ConfigurePS(0, 0, 720, 1280, 720, 4, kPXP_PsPixelFormatRGB888, (void *) Thermostat32bpp_Bitmap0);
  ConfigureAS(360-208, 640-208, 416, 416, 416, 4, kPXP_AsPixelFormatARGB8888, (void *) renderTarget[curVgBufferIdx^1U].memory);
  PXP_Start(APP_PXP);
  pxpFinish();
     
  uint32_t * outputPixel =  (uint32_t *)s_BufferLcd[curLcdBufferIdx];
  uint32_t * psPixel = (uint32_t *)Thermostat32bpp_Bitmap0;
     
  void * startingAddress = outputPixel + (114*720 + 114);
  ConfigureOB(144, 144, startingAddress);
  ConfigurePS(0, 0, 144, 144, 720, 4, kPXP_PsPixelFormatRGB888, startingAddress);
  ConfigureAS(0, 0, 144, 144, 144, 2, kPXP_AsPixelFormatARGB4444, (void *) LightsPXP_Bitmap0);
  PXP_Start(APP_PXP);
  pxpFinish();
  
  startingAddress = outputPixel + (114*720 + 464);
  ConfigureOB(144, 144, startingAddress);
  ConfigurePS(0, 0, 144, 144, 720, 4, kPXP_PsPixelFormatRGB888, startingAddress);
  ConfigureAS(0, 0, 144, 144, 144, 2, kPXP_AsPixelFormatARGB4444, (void *) CoolingPXP_Bitmap0);
  PXP_Start(APP_PXP);
  pxpFinish();
     
  startingAddress = outputPixel + (1017*720 + 122);
  ConfigureOB(128, 155, startingAddress);
  ConfigurePS(0, 0, 128, 155, 720, 4, kPXP_PsPixelFormatRGB888, startingAddress);
  ConfigureAS(0, 0, 128, 155, 128, 2, kPXP_AsPixelFormatARGB4444, (void *) SpeakerPXP_Bitmap0);
  PXP_Start(APP_PXP);
  pxpFinish();
     
  startingAddress = outputPixel + (1043*720 + 472);
  ConfigureOB(128, 103, startingAddress);
  ConfigurePS(0, 0, 128, 103, 720, 4, kPXP_PsPixelFormatRGB888, (void *) startingAddress);
  ConfigureAS(0, 0, 128, 103, 128, 2, kPXP_AsPixelFormatARGB4444, (void *) LockPXP_Bitmap0);
  PXP_Start(APP_PXP);
  pxpFinish();
  
  startingAddress = outputPixel + 672;
  ConfigureOB(48, 1280, startingAddress);
  ConfigurePS(0, 0, 48, 1280, 720, 4, kPXP_PsPixelFormatRGB888, (void *) startingAddress);
  ConfigureAS(0, 0, 48, 1280, 48, 2, kPXP_AsPixelFormatARGB4444, topBar.memory);
  PXP_Start(APP_PXP);
  pxpFinish();
